...
data "aws_caller_identity" "current" {
  count = ( var.create_recorder && var.enabled ) ? 1 : 0
}
  ...
  policy = <<POLICY
{
  ...
  "Statement": [
    ...
    },
    {
        ...
        "Effect": "Allow",
        "Action": "s3:PutObject",
        "Resource": "arn:aws:s3:::${local.bucket_name}/${local.s3_key_prefix}/AWSLogs/${data.aws_caller_identity.current[0].account_id}/Config/*",
        "Condition": {
          "StringLike": {
